...
AE::Graphics::Renderer::Shader::Shader(const std::string& vertexSrc, const std::string& fragmentSrc)
{
	...
	if (isCompiled == GL_FALSE)
	{
		...
		AE_CORE_ERROR("Vertex shader compilation failed: {0}", infoLog.data());
		return;
	}
	...
	if (isCompiled == GL_FALSE)
	{
		...
		AE_CORE_ERROR("Fragment shader compilation failed: {0}", infoLog.data());
		return;
	}
	...
	if (isLinked == GL_FALSE)
	{
		...
		AE_CORE_ERROR("Shader link failed: {0}", infoLog.data());
...
void AE::Graphics::Renderer::Shader::UploadUniformFloat(const std::string& name, const float value) const
{
	const GLint location = glGetUniformLocation(m_RendererID, name.c_str());
...
void AE::Graphics::Renderer::Shader::UploadUniformFloat4(const std::string& name, const glm::vec4& value) const
{
	const GLint location = glGetUniformLocation(m_RendererID, name.c_str());
